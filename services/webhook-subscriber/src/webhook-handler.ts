...
import {DatabaseHealthChecks, HealthChecker, SQSHealthChecks, ElasticsearchHealthChecks, ObservabilityFactory} from '@platform/infrastructure';
...
export class WebhookService {
    ...
    private async initializeApplication(configOptions?: any): Promise<void> {
        ...
        if (ConfigProvider.isInitialized()) {
            const logLevel = ConfigProvider.get(CONFIG.LOG_LEVEL) || 'debug';
        ...
        // Debug config dump
        const configs = ConfigProvider.get(CONFIG.ALL_CONFIG);
        logger.info('-==============================')
    ...
    private async runExecutionLoop(): Promise<void> {
        ...
        const messageProcessor = new HubspotWebhookConsumer(); // Use real HubSpot API, no mock injection
...
export async function callWebhookHandler(options?: any) {
    const service = new WebhookService();
    const port = 3001; // Default port, could be made configurable
    
    // Phase 1: Start health server (only acceptable crash point)
    await service.startServer(port);
    
    // Phase 2: Start application with deterministic behavior
    await service.startApplication({
        testMode: isTestMode(),  // Use same deterministic logic
        maxRetries: 3,
        retryDelayMs: 5000,
 ...
 */
function setupShutdownHandlers(service: WebhookService) {
...
if (import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith('webhook-handler.ts')) {
    main().catch(error => {
